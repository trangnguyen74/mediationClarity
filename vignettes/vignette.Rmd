...
This vignette is long-form documentation of the package `tnMediation`, and is also the computing appendix to the manuscript @nguyen2022MediationEstimation titled *Causal mediation analysis: from simple to more robust estimation strategies for marginal natural (in)direct effects.* (arxiv:2102.06048).
...
We will briefly describe the data (which will help make everything concrete) below.

Before getting to the estimators (what you are most likely here for), we will cover all the weighting matter in Section 2, as weighting is involved in multiple estimators. Sections 3-5 then track the three groups of estimators presented in the paper. For each weighting method or estimator, we present the function that implements the method and explain its outputs.
...
The additional Section 6 includes some manual code that shows the gist of each method, for the interested user. We recommend reading the paper to fully understand the methods and their properties.
...
w.med <- weights_med(
    ...
    plot        = TRUE,   # default
    c.vars.std  = "sfc0", # C and M vars for which want 
    m.vars.std  = "sfc",  # standardized mean diffs in balance plot
)
```
...
```{r, fig.height=3.5, fig.width=4, fig.align='center', fig.cap="Mediation weighting: distributions of weights"}
names(w.med$plots)
...
There are times when we might want to use the same model form for several models (like what we do in the code above). We can use shortcut arguments `y.c.form` and `y.cm.form`:
...
# Estimators -- if want additive effects
...
# Code for results in manuscript
