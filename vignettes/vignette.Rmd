---
title: "tnMediation: simple and more robust estimation of marginal natural (in)direct effects"
output: 
...
This vignette is long-form documentation of the package `tnMediation`, and is also the computing appendix to the manuscript 

Nguyen et al. (2022). Causal mediation analysis: from simple to more robust estimation strategies for marginal natural (in)direct effects. arxiv:2102.06048.

\

We will briefly describe the data (which help make everything concrete) in Section 2.

\

Before getting to the estimators (what you are most likely here for), we will cover all the weighting matter in Section 3. The *mediation weighting* applies to multiple estimators, so it makes sense to get it clear first.

\

Sections 4-6 then track the three groups of estimators presented in the paper.

\

For each weighting method or each estimator, this vignette includes three elements, in the following order:

- shows code that manually computes the weights or obtains the point estimate for the estimator. This code is not meant to be used, its purpose is simply to explain the method.
- presents the specific function from the package that should be used to implement the weighting or estimator and explain its arguments.
- shows the outputs of the function.

\

To speed up the compling of this document, we put the number of bootstrap samples in each estimator function call at 99, which is clearly too small. The defaul in the package is 999.

\

# A bit about data

We use a synthetic dataset `synth` that was created based on the PAS trial. It is available when you load the package. See description in the paper. 
...
Mediation weighting is weighting subsamples to create pseudo treated sample (herein `p11`) and pseudo control sample (`p00`) that mimic the C distribution of the full sample, and to create pseudo cross-world sample(s) (`p10` and/or `p01`) that mimic the full sample C distribution and the M given C distribution of the controls (if `p10`) or the treated (if `p01`).

Mediation weighting is used for the pure weighting estimator (wtd), the more robust estimators (Y2pred.R, psYpred.MR, Ypred.MR, MsimYpred.MR), and the estimators that use the three pseudo samples and working regression to improve precision (wt-Cadj2 and wt-Cadj1).

Following the paper, we show all computation just for the (NDE0, NIE1) decomposition, which requires `p10` but not `p01`.

The paper mentions three formulas for the cross-world weights. We use the second formula, based on fitting two models for P(A|C) and P(A,C,M).
...
But we could simply use the package, specifically function `weights_med()`. The advantage is that it automatically makes weight-distribution and balance plots. Also, it accommodates sampling weights.
...
`cross.world` = "10" is used for the (NDE0, NIE1) decomposition; the other options are "01" and "both". 

`c.vars.std` and `m.vars.std` specify covariates and mediators for which to use standardized (instead of unstandardized) mean differences in the balance plot.

If there are sampling weights, provide name of the sampling weights variable in argument `s.wt.var`.
...
`w.dat` is the weighted dataset. It has several new variables, whose names all start with a dot to differentiate them from the original data. Three of these variables are for weights: `.s.wt` (sampling weights), `.w.wt` (distribution-morphing weights) and `.f.wt` (final weights, the product of sampling and distribution-morphing weights). In this example, we do not have sampling weights, so variable `.s.wt` = 1 and `.f.wt` = `.w.wt`.
...
If the original data has sampling weights, there will be two of these plots, one for `.w.wt`, one for `.f.wt`.
...
For the continuous covariate `sfc0` and continuous mediator `sfc`, we would want to also look at distributional balance. We do not implement distributional balance in the current package. Distributional balance could be implemented using packages that focus on balance visualization, eg the `cobalt` package [CITE].
...
Here is the code for manual implementation of C-based inverse probability weighting.
...
And here is the code for manual implementation of CM-based odds weighting.
...
For `weights_cm.odds()`, the optional `cm.order` argument specifies the order by which variables appear in the balance plot. We use this argument because the plotting order we want (first covariates then mediators) does not match the order by which variables appear in `a.cm.form`.

Similarly, `weights_med()` has arguments `c.order` and `m.order` and `weights_ipw()` has `c.order`; we just didn't need to use them.
...
### Manual code (point estimation only, not run)

The code chunk below obtains the point estimate of the pure weighting estimator wtd.
...
It is better to use the package, specifically the function `estimate_wtd()`. It provides both point estimate and confidence interval via bootstrapping.
...
The arguments are similar to the arguments of `weights_med()`, except we need to specify the outcome variable name (via `y.var`) and also have bootstrap inputs. 

Two options are available for `boot.method`: "resample" (resampling bootstrap) and "cont-wt" (continuous weights bootstrap). If `boot.seed` is not specified, the function randomly draws a seed.
...
The code chunk below obtains the point estimate of the nonrobust estimator Y2pred. 

For the robust Y2predR, we would first estimate weights to form the pseudo samples `p00`, `p11`, `p10`, then replace `s11` and `s00` with `p11` and `p00` in the estimation of regular PO means, and replace `s11` and `s00` with `p10` and `p00` in the estimation of $E[Y_{1M_0}]$.
...
`y.c0.form` and `y.c1.form` are model formulas for E[Y|C,A=0] and E[Y|C,A=1].

`y.cm1.form` and `y10.c.form` are model formulas for E[Y|C,M,A=1] and $E[Y_{1M_0}|C]$, which are required for `cross.world` = "10".
For `cross.world` = "01", use `y.cm0.form` and `y01.c.form`, for E[Y|C,M,A=0] and $E[Y_{0M_1}|C]$.

There are times when we might want to use the same model form for several models, (like what we do above in the code above). We can use shortcut arguments `y.c.form` and `y.cm.form`:

- the formula in `y.c.form` gets populated to `y.c0.form`, `y.c1.form`, `y10.c.form`, `y01.c.form` if any of these arguments are required but not specified
- the formula in `y.cm.form` gets populated to `y.cm0.form`, `y.cm1.form` if either of these arguments is required but not specified
...
# the more robust psYpred.MR estimator
...
The code chunk below obtains the point estimate of the nonrobust estimator Ypred.

For the more robust YpredMR estimator, we would first estimate weights to form the pseudo samples p00, p11, p10, and replace s11, s00 and s10 in fitting outcome models below with p11, p00 and p10.
...
# the more robust Ypred.MR estimator
...
# Estimators -- if additive effects

## NDEpred pair

### Manual code (point estimation only, not run)

The code chunk below obtains the point estimate of the nonrobust estimator NDEpred.

For the robust NDEpredR, we would first estimate weights to form the pseudo samples p00, p11, p10, then replace s11 and s00 with p11 and p00 in the estimation of TE, and replace s11 and s00 with p10 and p00 in the estimation of NDE0.
...
# the robust NDEpred.R estimator
...
The code chunk below obtains the point estimate of the nonrobust estimator wp-Cadj.

For the more robust wpMR-Cadj, we would first estimate weights to form the pseudo samples p00, p11, p10 (using the `weights_med()` function), then replace s11 with p10 in the fitting of the E[Y|C,M,A=1] model.
