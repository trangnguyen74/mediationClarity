...
The package `mediationClarity` implements the methods covered in the manuscript @nguyen2022MediationEstimation titled *Causal mediation analysis: from simple to more robust estimation strategies for marginal natural (in)direct effects.* (arxiv:2102.06048). The key documentation of the package is the paper itself, which contains all the technical details of the methods. This vignette, which presents the functions that implement the methods, is the computing appendix to the paper.
...
Section 6 contains code that, if run, produces the results in the manuscript.
