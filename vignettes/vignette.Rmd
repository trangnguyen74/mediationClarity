...
subtitle: "Computing appendix to Nguyen et al. (2022). arXiv:2102.06048."
output: 
...
This vignette is long-form documentation of the package `tnMediation`, and is also the computing appendix to Nguyen et al. (2022). arxiv:2102.06048.
...
To fasten the building of this document, we put the number of bootstrap samples in each call of an estimator function at 99, which is clearly too small. The defaul in the package is 999.
...
w.cm.odds <- weights_cm.odds(
    data        = synth,
    cross.world = "10",  # default
    a.cm.form   = "treat ~ sex + age + edu + religion + drink0 + att0 * att + rul0 * rul + splines::ns(sfc0, 4) * splines::ns(sfc, 4)",
    plot        = TRUE,  # default
    cm.vars.std = c("sfc0", "sfc"),
    cm.order    = c("sex", "age", "edu", "religion", "drink0", "rul0", "att0", "sfc0", "rul", "att", "sfc")
)
```
...
### Point estimation manual code (not run)
...
This balance plot is the same plot as that from `weights_med()`, but there are two subtle differences: 
